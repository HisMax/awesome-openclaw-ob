---
tags:
  - 入口
  - OpenClaw
aliases:
  - 首页
  - Home
---

# OpenClaw 知识库

这是一份关于 **OpenClaw** 的原子化知识库，涵盖产品架构、安全分析、竞品对比、生态社区、商业投资、趋势预测、使用案例、基础概念和关键人物。

> OpenClaw 是一个免费、开源的自主 AI Agent 框架，运行在用户本地设备上，通过 WhatsApp、Telegram、Discord 等即时通讯应用连接大语言模型来自主执行任务。

## 从这里开始

想了解 OpenClaw 本身，从 [[OpenClaw 是什么]] 开始，然后沿着双链自由探索。

几个核心入口：

- **架构**：[[Agent-Flow-Loop 原理]] → [[Agent Execution Loop]] → [[Tool Use 机制]]
- **安全**：[[安全边界与风险（总览）]] → [[致命三合一安全矛盾]] → [[ClawHavoc 事件]]
- **竞品**：[[竞品对比总览]] → [[Claude Code 分析]] → [[Cursor 分析]]
- **人物**：[[Peter Steinberger]] → [[Andrej Karpathy]] → [[Dario Amodei]]
- **趋势**：[[2026 Agent 元年]] → [[Agent 范式转变]] → [[半人马阶段理论]]
- **案例**：[[案例-华尔街级股票筛选器]] → [[案例-Telegram 聊天开发 iOS 应用]]
- **最新动态**：[[v2026.3.22 发布事件]] → [[OpenClaw v2026.3 版本更新]] → [[v2026.3.22 社区贡献]]
- **v3.22 架构**：[[ClawHub 原生安装]] → [[捆绑提供商插件化迁移]] → [[Gateway 编译插件加载]]
- **v3.22 安全**：[[沙箱运行时注入防护]] → [[SSRF 固定与代理安全]] → [[Marketplace 安装范围验证]]
- **v3.22 生态**：[[飞书集成]] → [[Anthropic Vertex 提供商]] → [[Xiaomi MiMo 提供商]]
- **v3.22 UI**：[[Control UI 主题系统]] → [[Expand-to-Canvas]] → [[WCAG 无障碍标准]]
- **2026 Q2 安全**：[[Cyera Research 与 Claw Chain 披露]] → [[GTIG 首次确认 AI 生成零日事件]]

## 知识库统计

| 指标 | 数量 |
|------|------|
| 笔记总数 | **351 篇** |
| 双向链接 | **3,250+** |
| 主题分区 | 9 个 |
| 人物与事件节点 | 18 篇 |

剩下的，打开 Graph View 自由发散吧。
